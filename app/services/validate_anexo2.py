...
from app.services.placeholders import build_placeholders_anexo2, build_rows_anexo2, build_atividades_rows, build_alteracoes_rows
...
def validate_and_enrich_anexo2(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not atv_tabela:
        errors.append({"field": "atividades_tabela", "message": "Adicione pelo menos uma linha na tabela de atividades."})
    else:
        ...
        if not tem_atividade_valida:
            errors.append({"field": "atividades_tabela", "message": "Preencha a descrição das atividades em pelo menos uma linha da tabela."})
    ...
    for i, t in enumerate(ida_list):
        if _norm_cidade(t.get("origem")) == _norm_cidade(t.get("destino")):
            errors.append({"field": f"afastamento.ida.{i}.destino", "message": "A origem e o destino não podem ser a mesma cidade."})
    for i, t in enumerate(ret_list):
        if _norm_cidade(t.get("origem")) == _norm_cidade(t.get("destino")):
            errors.append({"field": f"afastamento.retorno.{i}.destino", "message": "A origem e o destino não podem ser a mesma cidade."})

    for i in range(len(ida_list) - 1):
        if _norm_cidade(ida_list[i].get("destino")) != _norm_cidade(ida_list[i + 1].get("origem")):
            errors.append({"field": f"afastamento.ida.{i + 1}.origem", "message": f"O destino do trecho {i + 1} deve ser a origem do trecho {i + 2}."})
    for i in range(len(ret_list) - 1):
        if _norm_cidade(ret_list[i].get("destino")) != _norm_cidade(ret_list[i + 1].get("origem")):
            errors.append({"field": f"afastamento.retorno.{i + 1}.origem", "message": f"O destino do trecho {i + 1} deve ser a origem do trecho {i + 2}."})

    if ida_list and ret_list:
        if _norm_cidade(ida_list[-1].get("destino")) != _norm_cidade(ret_list[0].get("origem")):
            errors.append({"field": "afastamento.retorno.0.origem", "message": "O destino da ida deve ser o mesmo que a origem do retorno."})
    ...
    return {"ok": True, "flags": flags, "placeholders": placeholders, "rows": rows, "atividades_rows": atividades_rows, "alteracoes_rows": alteracoes_rows}
